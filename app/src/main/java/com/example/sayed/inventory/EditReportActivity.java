...
public class EditReportActivity extends AppCompatActivity
    ...
    private void saveReport(){
        ...
        if (mCurrentReportUri == null) {
            ...
            if (newUri == null) {
                Toast.makeText(this, "Error with save report",
                        Toast.LENGTH_LONG).show();
            }else {
                Toast.makeText(this, "saved report", Toast.LENGTH_LONG).show();
            }
        }else {
            int rowsAffected = getContentResolver().update(mCurrentReportUri, values,
                    null, null);
            if (rowsAffected == 0){
                Toast.makeText(this, "Error with update report", Toast.LENGTH_LONG).show();
            }else {
                Toast.makeText(this, "report updated successful", Toast.LENGTH_LONG).show();
            }
        }
    }
    @Override
