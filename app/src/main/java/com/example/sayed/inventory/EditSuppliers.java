...
public class EditSuppliers extends AppCompatActivity
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (mCurrentSupplierUri == null) {
            ...
            //واخفاء قائمة حذف منتج اذ ليس من المنطقي حذف منتج لم يتم تسجيله بعد
    ...
    private void saveReportSupplier(){
        ...
        if (mCurrentSupplierUri == null) {
            ...
            if (newUri == null) {
                Toast.makeText(this, "Error with save report supplier",
                        Toast.LENGTH_LONG).show();
            }else {
                Toast.makeText(this, "saved report supplier", Toast.LENGTH_LONG).show();
            }
        }else {
            int rowsAffected = getContentResolver().update(mCurrentSupplierUri, values,
                    null, null);
            if (rowsAffected == 0){
                Toast.makeText(this, "Error with update report supplier", Toast.LENGTH_LONG).show();
            }else {
                Toast.makeText(this, "report supplier updated successful", Toast.LENGTH_LONG).show();
            }
        }
    }
    @Override
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            ...
            case R.id.delete_supplier:
                ...
                // هل ترد حذف هذا منتج او لا وناخذ تحديد القيمة منه ونبني عليها كود جديد
    ...
    private void showDeleteConfirmationDialog() {
        ...
        builder.setMessage("delete this the report supplier?");
        // if selected delete
    ...
    private void deleteProduction() {
        if (mCurrentSupplierUri != null) {
            ...
            if (deleteRow == 0) {
                Toast.makeText(this, "Error with delete report", Toast.LENGTH_LONG).show();
            } else {
                Toast.makeText(this, "The report is deleted", Toast.LENGTH_LONG).show();
            }
        }
        // Close the activity
        finish();
